...
<CrossCourseRef
  ...
  lessonSlug="15-alternative-execution-engines/02-datafusion-comet"
  label="Spark + Comet: взгляд из JVM-мира"
  preview="Тот же Comet с точки зрения Spark — как настроить плагин, как читать explain, как мониторить fallback rate в production."
/>

<CrossCourseRef
  courseId="spark-course"
  lessonSlug="15-alternative-execution-engines/01-native-execution-foundations"
  label="Native execution в Spark: фундамент"
  preview="Почему Spark получает JVM-замены (Comet, Gluten + Velox/CH backend) и как это вписывается в эволюцию Spark 4.x."
/>
