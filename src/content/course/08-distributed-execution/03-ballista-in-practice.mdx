...
<DiagramContainer title="Удалённые компоненты Ballista 43.0.0" className="my-6">
  <Grid columns={2} className="gap-3 text-sm">
    <DataBox label="Web UI" className="border-red-500/30" tooltip="Встроенный веб-дашборд планировщика удалён — используйте внешний мониторинг (Prometheus, Grafana)">
    ...
    <DataBox label="Caching subsystem" className="border-red-500/30" tooltip="Подсистема кеширования промежуточных результатов удалена — реализуйте через кастомный TableProvider">
    ...
    <DataBox label="Plugin subsystem" className="border-red-500/30" tooltip="Динамическая загрузка расширений удалена — компилируйте расширения статически для стабильности">
    ...
    <DataBox label="KV stores" className="border-red-500/30" tooltip="Встроенные KV stores (etcd, sled) удалены — scheduler хранит состояние in-memory для простоты">
