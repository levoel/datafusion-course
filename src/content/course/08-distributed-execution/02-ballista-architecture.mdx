...
<DiagramContainer title="Архитектура Ballista" className="my-6">
  <FlowColumn className="gap-4 text-sm">
    <FlowRow className="gap-3 justify-center">
      <DataBox label="Rust SessionContext" className="border-blue-500/30">
        <div className="text-xs font-mono">SessionContext::remote()</div>
      </DataBox>
      <DataBox label="PyBallista" className="border-blue-500/30">
        <div className="text-xs font-mono">BallistaBuilder()</div>
      </DataBox>
      <DataBox label="Flight SQL / JDBC" className="border-blue-500/30">
        <div className="text-xs">SQL-клиенты</div>
      </DataBox>
    </FlowRow>
    <Arrow direction="down" label="gRPC / Flight SQL" />
    <DataBox label="Scheduler" className="border-emerald-500/30 mx-auto">
      <FlowRow className="gap-3 p-2">
        <FlowNode className="bg-emerald-950/60 text-xs p-1">Job Manager</FlowNode>
        <FlowNode className="bg-emerald-950/60 text-xs p-1">Execution Graph</FlowNode>
        <FlowNode className="bg-emerald-950/60 text-xs p-1">Task Scheduler</FlowNode>
      </FlowRow>
    </DataBox>
    <Arrow direction="down" label="gRPC: задачи (protobuf физ. планы)" />
    <FlowRow className="gap-3 justify-center">
      <DataBox label="Executor 1" className="border-amber-500/30">
        <div className="text-xs">DataFusion runtime</div>
        <div className="text-xs mt-1 font-mono">N потоков</div>
      </DataBox>
      <DataBox label="Executor 2" className="border-amber-500/30">
        <div className="text-xs">DataFusion runtime</div>
        <div className="text-xs mt-1 font-mono">N потоков</div>
      </DataBox>
      <DataBox label="Executor N" className="border-amber-500/30">
        <div className="text-xs">DataFusion runtime</div>
        <div className="text-xs mt-1 font-mono">N потоков</div>
      </DataBox>
    </FlowRow>
    <Arrow direction="down" label="Arrow IPC" />
    <FlowNode className="bg-gray-800/60 text-xs p-2 mx-auto">Shuffle-файлы (Arrow IPC на локальном диске executor-а)</FlowNode>
  </FlowColumn>
</DiagramContainer>
...
<DiagramContainer title="Arrow IPC shuffle между stage" className="my-6">
  <FlowColumn className="gap-3 text-sm">
    <FlowRow className="gap-3 justify-center">
      <FlowNode className="bg-amber-950/60 text-xs p-2">Stage 1, partition 0<br/>→ Arrow IPC файл</FlowNode>
      <FlowNode className="bg-amber-950/60 text-xs p-2">Stage 1, partition 1<br/>→ Arrow IPC файл</FlowNode>
      <FlowNode className="bg-amber-950/60 text-xs p-2">Stage 1, partition 2<br/>→ Arrow IPC файл</FlowNode>
    </FlowRow>
    <Arrow direction="down" label="сеть (executor → executor)" />
    <FlowRow className="gap-3 justify-center">
      <FlowNode className="bg-emerald-950/60 text-xs p-2">Stage 2, partition 0<br/>ShuffleReaderExec</FlowNode>
      <FlowNode className="bg-emerald-950/60 text-xs p-2">Stage 2, partition 1<br/>ShuffleReaderExec</FlowNode>
    </FlowRow>
  </FlowColumn>
</DiagramContainer>
