...
<DiagramContainer title="Каскад оптимизаций" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input">WHERE a > 5 AND true AND b = b</FlowNode>
    <Arrow direction="down" label="SimplifyExpressions" />
    <FlowNode variant="process">WHERE a > 5 AND true → WHERE a > 5</FlowNode>
    <Arrow direction="down" label="PushDownFilter" />
    <FlowNode variant="process">TableScan: t, filter=[a > 5]</FlowNode>
    <Arrow direction="down" label="OptimizeProjections" />
    <FlowNode variant="output">TableScan: t, projection=[a, b], filter=[a > 5]</FlowNode>
  </FlowColumn>
</DiagramContainer>
