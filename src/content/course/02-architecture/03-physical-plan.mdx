  ...
  <FlowColumn className="p-4 gap-2">
    ...
    <FlowRow className="gap-4 justify-center flex-wrap">
      <FlowColumn className="gap-1 items-center">
        ...
        <Arrow direction="down" />
        <FlowNode variant="input" tooltip="Чтение одного файла или row group Parquet — отдельная партиция данных">ParquetExec (part 0)</FlowNode>
      </FlowColumn>
      <FlowColumn className="gap-1 items-center">
        <FlowNode variant="process" tooltip="Частичная агрегация: вычисляет промежуточные аккумуляторы в своей партиции">AggregateExec (partial)</FlowNode>
        <Arrow direction="down" />
        <FlowNode variant="input" tooltip="Чтение второго файла Parquet параллельно с другими партициями">ParquetExec (part 1)</FlowNode>
      </FlowColumn>
      <FlowColumn className="gap-1 items-center">
        <FlowNode variant="process" tooltip="Частичная агрегация: вычисляет промежуточные аккумуляторы в своей партиции">AggregateExec (partial)</FlowNode>
        <Arrow direction="down" />
        <FlowNode variant="input" tooltip="Чтение третьего файла Parquet — все партиции исполняются параллельно через tokio">ParquetExec (part 2)</FlowNode>
      </FlowColumn>
    </FlowRow>
  </FlowColumn>
</DiagramContainer>
...
В этой схеме:
- `ParquetExec` создаёт по одной партиции на файл (или row group)
...
```
SortExec.next()          →  AggregateExec.next()    →  FilterExec.next()   →  ParquetExec.next()
