...
<DiagramContainer title="Query Pipeline DataFusion" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input" tooltip="Исходный SQL-запрос в текстовом виде — ещё не проверен на корректность">SQL Text</FlowNode>
    <Arrow direction="down" label="sqlparser-rs" />
    <FlowNode variant="process" tooltip="Абстрактное синтаксическое дерево: структура запроса без привязки к реальным таблицам">AST (Statement)</FlowNode>
    <Arrow direction="down" label="SqlToRel" />
    <FlowNode variant="process" tooltip="Дерево логических операций с проверкой таблиц, колонок и типов через каталог">Unoptimized LogicalPlan</FlowNode>
    <Arrow direction="down" label="Optimizer (правила)" />
    <FlowNode variant="process" tooltip="Оптимизированный план: после pushdown фильтров, проекций и упрощения выражений">Optimized LogicalPlan</FlowNode>
    <Arrow direction="down" label="PhysicalPlanner" />
    <FlowNode variant="process" tooltip="Дерево конкретных алгоритмов: HashJoin, SortExec, DataSourceExec с партиционированием">ExecutionPlan (дерево)</FlowNode>
    <Arrow direction="down" label="execute(partition)" />
    <FlowNode variant="output" tooltip="Асинхронный поток RecordBatch — финальный результат запроса">SendableRecordBatchStream</FlowNode>
  </FlowColumn>
</DiagramContainer>
...
<DiagramContainer title="LogicalPlan Tree (до оптимизации)" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="output" tooltip="Ограничивает количество строк результата — применяется последним">Limit (count=10)</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process" tooltip="Сортирует данные по указанному выражению — здесь по сумме по убыванию">Sort (total DESC)</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process" tooltip="Выбирает и переименовывает колонки для финального результата">Projection (region, total)</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process" tooltip="Группирует строки по ключу и вычисляет агрегатные функции (SUM, COUNT)">Aggregate (GROUP BY region, SUM(amount))</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process" tooltip="Оставляет только строки, удовлетворяющие предикату WHERE">Filter (status = 'completed')</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="input" tooltip="Leaf-узел: чтение данных из зарегистрированного источника">TableScan (orders)</FlowNode>
  </FlowColumn>
</DiagramContainer>
...
<CrossCourseRef
  ...
  lessonSlug="02-catalyst-tungsten/01-catalyst-overview"
  label="Spark Catalyst: тот же pipeline в JVM"
  preview="Catalyst Optimizer в Spark — параллельный взгляд на ту же 6-фазную модель (parsed → analyzed → optimized → physical) в JVM-мире."
/>

<CrossCourseRef
  courseId="clickhouse-course"
  lessonSlug="06-query-engine/01-vectorized-execution"
  label="ClickHouse: vectorized execution"
  preview="Как ClickHouse организует pipeline исполнения через chunks of columns — третий промышленный пример той же векторной модели."
/>
