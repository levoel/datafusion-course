...
<DiagramContainer title="Структура SessionContext" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <DataBox label="SessionContext" tooltip="Главная точка входа: координирует парсинг, планирование, оптимизацию и выполнение SQL" className="border-blue-500/30 w-72 mx-auto">
      <div className="text-xs font-mono">sql(), read_parquet(), register_table(), ...</div>
    </DataBox>
    <Arrow direction="down" />
    <FlowRow className="gap-4 justify-center flex-wrap">
      <DataBox label="SessionConfig" tooltip="Настройки поведения: batch_size, target_partitions, правила оптимизатора" className="border-yellow-500/30 w-48">
        <div className="text-xs font-mono">batch_size, target_partitions, optimizer rules, ...</div>
      </DataBox>
      <DataBox label="SessionState" tooltip="Состояние сессии: каталог таблиц, реестр UDF, оптимизатор и планировщик" className="border-emerald-500/30 w-48">
        <div className="text-xs font-mono">catalog, UDF registry, optimizer, planner</div>
      </DataBox>
      <DataBox label="RuntimeEnv" tooltip="Ресурсы выполнения: пул памяти, disk manager для spill, object store для S3/GCS" className="border-purple-500/30 w-48">
...
<DiagramContainer title="Pipeline: SQL → DataFrame → RecordBatch" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input" tooltip="Точка входа: SQL текст. SessionContext.sql() ничего не исполняет — лишь стартует ленивый pipeline.">ctx.sql("SELECT …")</FlowNode>
    <Arrow direction="down" />
    <DataBox label="SessionState — построение плана" tooltip="Lazy фаза: парсинг → логический план → оптимизация. Никаких I/O." className="w-80 mx-auto">
      <div className="text-xs space-y-1">
        <div>sql_to_statement() → AST</div>
        <div>statement_to_plan() → LogicalPlan</div>
        <div>optimize() → Optimized LogicalPlan</div>
      </div>
    </DataBox>
    <Arrow direction="down" />
    <FlowNode variant="process" tooltip="DataFrame обёртывает Optimized LogicalPlan. Терминальная операция (collect/show/write_parquet) запускает следующую стадию.">DataFrame (lazy)</FlowNode>
    <Arrow direction="down" />
    <DataBox label=".collect() — физическое исполнение" tooltip="Eager фаза: физический план + исполнение. Здесь происходят все I/O и вычисления." className="w-80 mx-auto">
      <div className="text-xs space-y-1">
        <div>create_physical_plan() → ExecutionPlan</div>
        <div>ExecutionPlan::execute() → RecordBatch stream</div>
      </div>
    </DataBox>
  </FlowColumn>
</DiagramContainer>
