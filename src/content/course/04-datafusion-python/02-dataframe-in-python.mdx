...
<DiagramContainer title="DataFrame: цепочка трансформаций в Python" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input" tooltip="Чтение Parquet-файла — создаёт ленивый DataFrame без загрузки данных">ctx.read_parquet("orders.parquet")</FlowNode>
    <Arrow direction="down" label=".filter()" />
    <FlowNode variant="process" tooltip="Фильтрация — добавляет узел Filter в LogicalPlan">DataFrame (LogicalPlan: Filter)</FlowNode>
    <Arrow direction="down" label=".select()" />
    <FlowNode variant="process" tooltip="Проекция — выбор и вычисление колонок в плане">DataFrame (LogicalPlan: Projection)</FlowNode>
    <Arrow direction="down" label=".aggregate()" />
    <FlowNode variant="process" tooltip="Агрегация — группировка и вычисление итогов (SUM, AVG, COUNT)">DataFrame (LogicalPlan: Aggregate)</FlowNode>
    <Arrow direction="down" label=".collect()" />
    <FlowNode variant="output" tooltip="Список PyArrow RecordBatch — результат выполнения всего плана">List[RecordBatch]</FlowNode>
  </FlowColumn>
</DiagramContainer>
...
<CrossCourseRef
  ...
  lessonSlug="10-data-libraries-conceptual/03-pyarrow-concept"
  label="PyArrow: Python ↔ Arrow мост"
  preview="Как pyarrow управляет памятью, что такое Table/RecordBatch в Python и как zero-copy работает с pandas/Polars/DataFusion-Python."
/>

<CrossCourseRef
  courseId="python-course"
  lessonSlug="10-data-libraries-conceptual/06-decision-matrix"
  label="Decision matrix: pandas / Polars / DuckDB / DataFusion-Python"
  preview="Когда выбирать DataFusion-Python вместо pandas/Polars/DuckDB для single-node OLAP-нагрузок."
/>
