...
<DiagramContainer title="SQL + DataFrame: комбинированный pipeline" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input">ctx.sql("SELECT ... FROM orders WHERE ...")</FlowNode>
    <Arrow direction="down" label="DataFrame" />
    <FlowNode variant="process">.filter(col("status").eq(lit("completed")))</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process">.aggregate([...], [...])</FlowNode>
    <Arrow direction="down" />
    <FlowNode variant="process">.sort(...).limit(10)</FlowNode>
    <Arrow direction="down" label=".show()" />
    <FlowNode variant="output">Результат</FlowNode>
  </FlowColumn>
</DiagramContainer>
