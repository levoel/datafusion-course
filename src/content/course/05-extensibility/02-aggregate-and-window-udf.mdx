...
<DiagramContainer title="Accumulator: жизненный цикл" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input" tooltip="DataFusion создаёт новый экземпляр Accumulator для каждой партиции данных">Создание Accumulator</FlowNode>
    <Arrow direction="down" label="Partition 1" />
    <FlowNode variant="process" tooltip="Первый батч строк обновляет внутреннее состояние аккумулятора — суммы, счётчики, коллекции">update_batch(values)</FlowNode>
    <Arrow direction="down" label="Partition 2" />
    <FlowNode variant="process" tooltip="Каждый следующий батч инкрементально обновляет состояние без пересоздания аккумулятора">update_batch(values)</FlowNode>
    <Arrow direction="down" label="Слияние партиций" />
    <FlowNode variant="process" tooltip="Промежуточные состояния из параллельных партиций объединяются в единый аккумулятор">merge_batch(states)</FlowNode>
    <Arrow direction="down" label="Финал" />
    <FlowNode variant="output" tooltip="Финальное вычисление преобразует внутреннее состояние в одно скалярное значение результата">evaluate() → ScalarValue</FlowNode>
  </FlowColumn>
</DiagramContainer>
...
<DiagramContainer title="Типы пользовательских функций" className="my-6">
  <Grid columns={3} className="gap-4 text-sm">
    <DataBox label="Scalar UDF" className="border-blue-500/30" tooltip="Скалярная функция: вычисляет одно значение для каждой входной строки — map-операция над массивами">
    ...
    <DataBox label="Aggregate UDAF" className="border-emerald-500/30" tooltip="Агрегатная функция: сводит N строк к одному значению через Accumulator с поддержкой параллельного слияния">
    ...
    <DataBox label="Window UDWF" className="border-yellow-500/30" tooltip="Оконная функция: вычисляет значение для каждой строки в контексте окна через PartitionEvaluator">
