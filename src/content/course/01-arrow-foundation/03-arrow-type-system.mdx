...
<DiagramContainer title="List<Int32>: [[1, 2], [3], NULL, [4, 5, 6]]" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <DataBox label="Validity Bitmap" tooltip="Битовая маска для вложенного List: отмечает NULL-элементы среди списков" className="border-yellow-500/30 w-64 mx-auto">
      <div className="text-xs font-mono text-center">[1, 1, 0, 1]</div>
    </DataBox>
    <Arrow direction="down" />
    <DataBox label="Offset Buffer (5 x int32)" tooltip="Определяет границы каждого списка в child array. Для NULL — пустой диапазон" className="border-purple-500/30 w-64 mx-auto">
      <div className="text-xs font-mono text-center">[0, 2, 3, 3, 6]</div>
    </DataBox>
    <Arrow direction="down" />
    <DataBox label="Child Array (Int32, 6 значений)" tooltip="Плоский массив всех элементов всех списков. Границы определяются offset buffer" className="border-blue-500/30 w-64 mx-auto">
...
<DiagramContainer title="Иерархия: Schema → Field → DataType" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <DataBox label="Schema" tooltip="Упорядоченный набор полей с метаданными. Определяет структуру RecordBatch" className="border-slate-400/30 w-72 mx-auto">
    ...
    <FlowRow className="gap-3 justify-center flex-wrap">
      <DataBox label='Field "id"' tooltip="Non-nullable целочисленное поле — идентификатор записи" className="border-emerald-500/30 w-48">
        <div className="text-xs space-y-1">
          <div>DataType: Int64</div>
          <div>nullable: false</div>
        </div>
      </DataBox>
      <DataBox label='Field "name"' tooltip="Nullable строковое поле — UTF-8 текст переменной длины" className="border-blue-500/30 w-48">
        <div className="text-xs space-y-1">
          <div>DataType: Utf8</div>
          <div>nullable: true</div>
        </div>
      </DataBox>
      <DataBox label='Field "tags"' tooltip="Вложенный тип: список строк. Child array хранит элементы, offset buffer определяет границы" className="border-purple-500/30 w-48">
        <div className="text-xs space-y-1">
