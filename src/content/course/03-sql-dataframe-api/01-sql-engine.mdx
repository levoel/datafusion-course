...
<DiagramContainer title="SQL → LogicalPlan Pipeline" className="my-6">
  <FlowColumn className="p-4 gap-2">
    <FlowNode variant="input" tooltip="Входной SQL-запрос от пользователя — текстовая строка">SQL Text</FlowNode>
    <Arrow direction="down" label="sqlparser-rs (парсинг)" />
    <FlowNode variant="process" tooltip="Абстрактное синтаксическое дерево — структурное представление SQL без проверки схемы">AST (Statement)</FlowNode>
    <Arrow direction="down" label="SqlToRel (семантический анализ)" />
    <FlowNode variant="process" tooltip="Логический план до оптимизации — привязан к каталогу таблиц и типов">Unoptimized LogicalPlan</FlowNode>
    <Arrow direction="down" label="Optimizer (цепочка правил)" />
    <FlowNode variant="output" tooltip="Оптимизированный план — pushdown фильтров, elimination колонок, constant folding">Optimized LogicalPlan</FlowNode>
  </FlowColumn>
</DiagramContainer>
